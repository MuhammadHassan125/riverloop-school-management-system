      ...
      <aside className="h-screen">
        <nav className="h-full flex flex-col bg-primaryBackground border-r overflow-x-hidden shadow-sm transition-all duration-300">
          ...
          <div className="border-t flex p-3">
            <div
              className={`flex justify-between items-center overflow-hidden transition-all duration-300 ${isSidebarOpen ? "w-52 ml-3" : "w-0"
                }`}
            >
  ...
  return (
    <li className="relative flex items-center py-1 px-0 my-1 font-medium rounded-md cursor-pointer transition-colors duration-300 group">
      ...
      >
        ...
        {alert && (
          <div
            className={`absolute right-2 w-4 h-4 text-white rounded bg-primaryPurple transition-all duration-300 ${isSidebarOpen ? "" : "top-2"
              }`}
          ></div>
        )}
      </Link>
      {!isSidebarOpen && (
        <div
          className={`absolute left-full z-30 rounded-md px-2 py-1 ml-6 bg-indigo-100 text-primaryPurple text-sm invisible opacity-20 -translate-x-3 transition-all duration-300 group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}
        >
          {text}
        </div>
      )}
